...
public class CorruptionTester {
	...
	public static void main(String[] args) {
		...
		result = PacketCorrupter.corruptPacket(packet, ErrorSimState.INVALID_OPCODE);
		System.out.println("Invalid Opcode:       " + TFTPPacket.toString(Arrays.copyOfRange(result, 0,result.length)) + "\n");
		result = PacketCorrupter.corruptPacket(packet, ErrorSimState.DATA_OR_ACK_INVALID_BLOCK_NUMBER);
		System.out.println("Invalid Block Number: " + TFTPPacket.toString(Arrays.copyOfRange(result, 0,result.length)) + "\n");
		result = PacketCorrupter.corruptPacket(packet, ErrorSimState.DATA_OR_ACK_MISSING_BLOCK_NUMBER);
		System.out.println("Missing Block Number: " + TFTPPacket.toString(Arrays.copyOfRange(result, 0,result.length)) + "\n");
		result = PacketCorrupter.corruptPacket(packet, ErrorSimState.DATA_MISSING_DATA);
		...
		result = PacketCorrupter.corruptPacket(packet, ErrorSimState.INVALID_OPCODE);
		System.out.println("Invalid Opcode:        " + TFTPPacket.toString(Arrays.copyOfRange(result, 0,result.length)) + "\n");
		result = PacketCorrupter.corruptPacket(packet, ErrorSimState.ERROR_INVALID_ERROR_CODE);
		System.out.println("Invalid Error Code:    " + TFTPPacket.toString(Arrays.copyOfRange(result, 0,result.length)) + "\n");
		result = PacketCorrupter.corruptPacket(packet, ErrorSimState.ERROR_MISSING_ERROR_CODE);
		System.out.println("Missing Error Code:    " + TFTPPacket.toString(Arrays.copyOfRange(result, 0,result.length)) + "\n");
		result = PacketCorrupter.corruptPacket(packet, ErrorSimState.ERROR_MISSING_ERROR_MESSAGE);
		System.out.println("Missing Error Message: " + TFTPPacket.toString(Arrays.copyOfRange(result, 0,result.length)) + "\n");
		result = PacketCorrupter.corruptPacket(packet, ErrorSimState.ERROR_MISSING_ZERO);
